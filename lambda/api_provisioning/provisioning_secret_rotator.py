"""Rotates Secrets Manager Entry for VIDEON_PROVISIONING_SECRET.
...
def lambda_handler(event, context):  # pylint: disable=unused-argument
    ...
    Args:
        event (dict): Lambda dictionary of event parameters.
          These keys must include the following:
            - SecretId: The secret ARN or identifier
            - ClientRequestToken: The ClientRequestToken of the secret version
            - Step: The rotation step
...
def create_secret(service_client, arn, token):
    ...
    Args:
        ...
        token (string): The ClientRequestToken associated with the secret
          version
    Raises:
        ResourceNotFoundException: If the secret with the specified
          arn and stage does not exist
    """
...
def set_secret():
    """Set the secret
    This method should set the AWSPENDING secret in the service that the secret
    belongs to.For example, if the secret is a database  credential, this
    method should take the value of the AWSPENDING secret and set the user's
    password to this value in the database.
    Args:
        service_client (client): The secrets manager service client
        arn (string): The secret ARN or other identifier
        token (string): The ClientRequestToken associated with the secret
...
def test_secret():
    """Test the secret
    This method should validate that the AWSPENDING secret works in the service
     that the secret belongs to. For example, if the secret is a database
     credential, this method should validate that the user can login with the
     password in AWSPENDING and that the user has
    ...
    logger.info(
        "testSecret: Skipping because this is a serverless application that "
        "does not have anything to test")
...
def finish_secret(service_client, arn, token):
    ...
    logger.info(
        "finishSecret: Successfully set AWSCURRENT stage to "
        "version %s for secret %s.", token, arn)
